const DATA = {
  map: {
    ...
    "tesla": { // electrifies enemies within range
      price: 3000,
      targets: 2,
      range: 1.5,
      speed: 3,
    ...
    "poisoner": { // poisons in a radius of [range] and applies damage to enemies
      price: 5000,
      targets: 1000,
      range: 1.5,
      speed: 3,
    ...
    "sniper": { // shoots an enemy from anywhere
      price: 6000,
      targets: 1,
      range: 100,
      speed: 0.1,
    ...
    "inferno": { // targets one enemy and increases in damage over time until enemy is dead or out of range
      price: 4000,
      targets: 1,
      range: 2,
      speed: 50,
    ...
    "mortar": { // targets one specific square on the map and fires onto it
      price: 2000,
      targets: 100,
      range: 100,
      speed: 2,
    ...
    "radar": { // freezes enemies in range and does a bit of damage
      price: 4000,
      targets: 1000,
      range: 1.5,
      speed: 0.5,
    ...
    "laser": { // fires a laser across the whole map in a straight non-diagonal line
      price: 2000,
      targets: 1000,
      range: 100,
      speed: 2,
...
function draw() {
  ...
  for (let i of Object.keys(GAME.map)) {
    for (let j of Object.keys(GAME.map[i])) {
      if (GAME.map[i][j]) {
        image(VISUALS.buildings[GAME.map[i][j].name][GAME.map[i][j].angle], 65 + i * 120 - 120, 65 + j * 120 - 120, 80, 80)
      }
    }
  }
  ...
  for (let i of GAME.enemies) {
    ...
    image(VISUALS.enemies[i.type][i.facing], 65 + (i.coords.split("-")[0] - 1) * 120, 65 + (i.coords.split("-")[1] - 1) * 120)
  }
  ...
  for (let i of animations) {
    if (i.split(":")[2] > 0) {
      image(VISUALS.animations[i.split(":")[0]], 65 + i.split(":")[1].split("-")[0] * 120 - 120, 65 + i.split(":")[1].split("-")[1] * 120 - 120)
  ...
  // show tower menu
    ...
    if (typeof tower.levels.speed != "boolean") {
      ...
      if (tower.levels.speed < 6) {
        ...
        text((tower_data.speed + tower_data.speed / 4 * tower.levels.speed).toFixed(2) + ">" + (tower_data.speed + tower_data.speed / 4 * (tower.levels.speed + 1)).toFixed(2) + "\nLVL: " + tower.levels.speed + "/6", 1212, 923)
      } else {
    ...
    if (typeof tower.levels.damage != "boolean") {
      ...
      if (tower.levels.damage < 6) {
        ...
        text((tower_data.damage + tower_data.damage / 4 * tower.levels.damage).toFixed(2) + ">" + (tower_data.damage + tower_data.damage / 4 * (tower.levels.damage + 1)).toFixed(2) + "\nLVL: " + tower.levels.damage + "/6", 1734, 923)
      } else {
    ...
    text(parseInt(tower_data.price + (tower.levels.speed * tower_data.price / 2 + tower_data.price * (tower.levels.speed * tower.levels.speed / 2) + (tower.levels.damage * tower_data.price / 2 + tower_data.price * (tower.levels.damage * tower.levels.damage / 2))) / 4) + "$", 1473, 836)

    fill("#FF0000")
    if (tower.name == "mortar") {
      text("SELECT\nTARGET", 1473, 923)
      if (tower.active_target) {
        image(VISUALS.elements.cross, 75 + (tower.active_target.split("-")[0] - 1) * 120, 75 + (tower.active_target.split("-")[1] - 1) * 120, 60, 60)
      }
  ...
  if (typeof GAME.enemies_left == "undefined") { GAME.context = 6; draw_loop = false; GAME.win=true; return }
      ...
      if (tower) {
        ...
        if (tower.cooldown <= 0) {
          ...
          for (let k of GAME.enemies) {
            if (Math.pow(Math.abs(k.coords.split("-")[0] - i), 2) + Math.pow(Math.abs(k.coords.split("-")[1] - j), 2) < Math.pow(tower_data.range, 2) && (!tower.targetted.includes(k) || tower.name == "laser")) {
              if (tower.name == "inferno" && k.type != "boss_goon" && k.type != "mega_goon") {
                if (tower.active_target === false) { tower.active_target = k.coords }
                else if (k.coords != tower.active_target) { continue }
                k.freeze = true
              } else if (tower.name == "mortar") {
                if (typeof tower.active_target !== "string") { break }
                ...
                if (!(Math.abs(k.coords.split("-")[0] - tower.active_target.split("-")[0]) < 0.5 && Math.abs(k.coords.split("-")[1] - tower.active_target.split("-")[1]) < 0.5)) {
                  continue
                }
              } else if (tower.name == "laser") {
                stroke("#00FFFF")
                switch (8 - tower.angle) {
                  ...
                  case 2:
                    line(105 + i * 120 - 120, 105 + j * 120 - 120, 105 + i * 120 - 120, 105 + j * 120 - 120 + (120 * (8 - j)))
                    break;

                  case 4:
                    line(105 + i * 120 - 120, 105 + j * 120 - 120, 105 + i * 120 - 120 - (120 * (i - 1)), 105 + j * 120 - 120)
                    break;

                  case 6:
                    line(105 + i * 120 - 120, 105 + j * 120 - 120, 105 + i * 120 - 120, 105 + j * 120 - 120 - (120 * (j - 1)))
                    break;

                  case 8:
                    line(105 + i * 120 - 120, 105 + j * 120 - 120, 105 + i * 120 - 120 + (120 * Math.min(8 - i)), 105 + j * 120 - 120)
                    break;
                }
                noStroke()
                ...
                if (tower.targetted.includes(k)) { continue }
              ...
              if (tower.name != "laser") {
                tower.angle = angle_translation(Math.atan2(k.coords.split("-")[1] - j, k.coords.split("-")[0] - i) * 180 / Math.PI - 45)
              }
              if (tower.name == "inferno") {
                stroke("#FF7F00")
                line(105 + i * 120 - 120, 105 + j * 120 - 120, 105 + k.coords.split("-")[0] * 120 - 120, 105 + k.coords.split("-")[1] * 120 - 120)
                noStroke()
              }
              tower.remaining_targets--
              tower.targetted.push(k)
              if (tower.animation != "none") { animations.push(tower.animation + ":" + k.coords + ":" + "5") }
...
function mousePressed() {
  ...
  if (mouseX < 1050 && ((mouseX - 65) % 120) < 81 && ((mouseY - 65) % 120) < 81) {
    if (!GAME.mortar_target_select) {
      ...
      GAME.map[GAME.selected.x + 1][GAME.selected.y + 1].active_target = (((mouseX - 65) - (mouseX - 65) % 120) / 120 + 1) + "-" + (((mouseY - 65) - (mouseY - 65) % 120) / 120 + 1)
  ...
  if (GAME.context === 2) {
    ...
    if (mouseX > 1090 && mouseX < 1332 && mouseY > 686 && mouseY < 841) {
      ...
      let tower_data = DATA.building[tower.name]
      if (!tower.levels.speed) { tower.levels.speed = 0 }
      if (!tower.levels.damage) { tower.levels.damage = 0 }
      GAME.money += parseInt(tower_data.price + (tower.levels.speed * tower_data.price / 2 + tower_data.price * (tower.levels.speed * tower.levels.speed / 2) + (tower.levels.damage * tower_data.price / 2 + tower_data.price * (tower.levels.damage * tower.levels.damage / 2))) / 4)
